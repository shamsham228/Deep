...
# --- MERGED: Fast Camera Imports from Code 2 ---
try:
    from modules.camera_fast import get_frame as get_cam_frame, get_fps as get_cam_fps
    HAS_FAST_CAM = True
except ImportError:
    HAS_FAST_CAM = False
    print("Warning: camera_fast module not found. Reverting to standard FPS calculation.")
...
# --- MERGED: Globals Reference ---
# Used as reference if needed by external modules
if hasattr(globals, 'latest_cam_frame'):
    frame = globals.latest_cam_frame
else:
    frame = None
...
def create_root(start: Callable[[], None], destroy: Callable[[], None]) -> ctk.CTk:
    ...
    # --- Camera Selection (Kept from Code 1 as it is more robust) ---
...
def create_webcam_preview(camera_index: int):
    ...
    while True:
            ...
            for frame_processor in frame_processors:
                if frame_processor.NAME == "DLC.FACE-ENHANCER":
                    if modules.globals.fp_ui["face_enhancer"]:
                        temp_frame = frame_processor.process_frame(None, temp_frame , provider="cpu")
                else:
                    temp_frame = frame_processor.process_frame(source_image, temp_frame, provide ="cpu")
        else:
            modules.globals.target_path = None
            for frame_processor in frame_processors:
                if frame_processor.NAME == "DLC.FACE-ENHANCER":
        ...
        # --- MERGED: FPS Logic ---
        # Logic: If 'camera_fast' module is available (Code 2), use it for FPS overlay.
        # Otherwise, fallback to Code 1's calculation.
        
        current_time = time.time()
        frame_count += 1
        
        if HAS_FAST_CAM:
            # Code 2 style: Get FPS from the fast camera module
            fps_to_show = get_cam_fps()
        else:
            # Code 1 style: Manual calculation
            if current_time - prev_time >= fps_update_interval:
                fps = frame_count / (current_time - prev_time)
                frame_count = 0
                prev_time = current_time
            fps_to_show = f"{fps:.1f}"
